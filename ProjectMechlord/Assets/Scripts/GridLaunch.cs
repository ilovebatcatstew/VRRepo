...
public class GridLaunch : MonoBehaviour
{
    ...
    void  DebugLaunchVolley()
    {
       ...
       // Debug.DrawLine(objectSpawnPosition.position, objectSpawnPosition.position + objectSpawnPosition.forward * 3, Color.white);

        for(int x = 0; x < columns - 1; x++)
        {
            for(int y = 0; y > -rows + 1; y--)
            {
            Debug.DrawLine(gameObject.transform.position + xStart + (xOffset * x) + yStart + (yOffset * y) + zStart, gameObject.transform.position + xStart + (xOffset * x) + yStart + (yOffset * y) + zStart + gameObject.transform.forward * 3, Color.white);
